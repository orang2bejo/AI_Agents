...
class PowerShellManager:
    ...
    def execute_command(
        ...
        try:
            ...
            if working_directory:
                ps_cmd[-1] = f"Set-Location '{working_directory}'; {command}"
            ...
            if ps_result.success:
                self.logger.info(
                    f"Command executed successfully in {execution_time:.2f}s"
                )
            else:
                self.logger.error(
    ...
    def get_running_processes(
        ...
        command += (
            " | Select-Object Name, Id, CPU, WorkingSet, StartTime | ConvertTo-Json"
        )
    ...
    def test_network_connection(
        self, target: str, port: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    if name in COMMON_SNIPPETS:
        try:
            ...
            logging.getLogger(__name__).error(
                f"Missing parameter for snippet {name}: {e}"
            )
            return None
    return None
